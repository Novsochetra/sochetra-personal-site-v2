...
import { ContainerPresentationProps } from "./Container.interfaces";
import styles from "./Container.module.css";

export const ContainerPresentation: React.FC<ContainerPresentationProps> = ({
  children,
}) => {
  useEffect(() => {
    ...
    if (process.browser) {
      ...
      // frame.setAttribute("height", window.innerHeight + "px");
      ...
      function createParticles(amount) {
        for (var i = 1; i <= amount; i++) {
          particles[i] = {
            dom: document.createElement("particle-" + getRandomInt(1, 2)),
            ...
            "x-velocity":
              getRandomInt(-max_velocity * 100, max_velocity * 100) / 100,
            "y-velocity":
              getRandomInt(-max_velocity * 100, max_velocity * 100) / 100,
          };

          particles[i]["dom"].style.cssText =
            "background-color: " +
            (getRandomInt(0, 2) == 1 ? "#cc1e2c;" : "#ff5434;") +
            "position: absolute;" +
            "opacity: " +
            particles[i]["opacity"] +
            "; border-radius: " +
      ...
      function animateParticles() {
        for (var i = 1; i < particles.length; i++) {
          ...
          if (
            particles[i]["left"] + particles[i]["size"] < 0 &&
            particles[i]["x-velocity"] < 0
          ) {
            particles[i]["dom"].style.left =
              canvas_width + particles[i]["x-velocity"] + "px";
            particles[i]["dom"].style.top =
              particles[i]["top"] + particles[i]["y-velocity"] + "px";

            // particle hits top side
          } else if (
            particles[i]["top"] + particles[i]["size"] < 0 &&
            ...
            particles[i]["dom"].style.top =
              ...
              "px";
            particles[i]["dom"].style.left =
              particles[i]["left"] + particles[i]["x-velocity"] + "px";

            // particle hits right side
          } else if (
            particles[i]["left"] > canvas_width &&
            particles[i]["x-velocity"] > 0
          ) {
